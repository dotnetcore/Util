...
using Util.Applications.Infrastructure;
...
namespace Util.Applications.Tests {
    ...
    public class CustomerControllerTest : TestBase {
        ...
        public async Task TestPageQueryAsync() {
            ...
            //创建5个实体,前4个设置了名称,并使用Code属性排序
            var dtos = CustomerFakeService.GetCustomerDtos( 5 ).OrderBy( t => t.Code ).ToList();
            for ( int i = 0; i < 4; i++ ) {
                dtos[i].Name = name;
            }
            ...
            //使用名称查询第二页,每页2行,并使用Code属性排序
            var query = new CustomerQuery { Page = 2, PageSize = 2, Name = name,Order = "Code" };
