...
public class DaprRunCommandTest {
    ...
    /// 测试初始化
    /// </summary>
    public DaprRunCommandTest( ITestOutputHelper output, ILogger<DaprRunCommandTest> logger ) {
        _output = output;
        _logger = logger;
    }
    ...
    public void TestExecute() {
        DaprRunCommand.Create( "app" )
            .Log( _logger )
            .Project( "../../../../../test/Util.Microservices.Dapr.WebApiSample/Util.Microservices.Dapr.WebApiSample.csproj" )
            .UseFreePorts()
            .Execute();
        DaprStopCommand.Create( "app" ).Log( _logger ).Execute();
    }
}