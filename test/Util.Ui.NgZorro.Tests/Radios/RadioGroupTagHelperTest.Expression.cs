...
namespace Util.Ui.NgZorro.Tests.Radios {
    ...
    public partial class RadioGroupTagHelperTest {
        ...
        public void TestFor_Bool() {
            ...
            result.Append( "<nz-radio-group #enabled=\"\" #x_enabled=\"xSelectExtend\" name=\"enabled\" nzName=\"enabled\" x-select-extend=\"\" [(ngModel)]=\"model.enabled\" " );
            result.Append( "[data]=\"[{'text':'是','value':true,'sortId':1},{'text':'否','value':false,'sortId':2}]\"" );
            result.Append( ">" );
            result.Append( "<label *ngFor=\"let item of x_enabled.options\" nz-radio=\"\" [nzDisabled]=\"item.disabled\" [nzValue]=\"item.value\">" );
        ...
        public void TestFor_Enum() {
            ...
            result.Append( "</nz-form-label>" );
            result.Append( "<nz-form-control>" );
            result.Append( "<nz-radio-group #gender=\"\" #x_gender=\"xSelectExtend\" name=\"gender\" nzName=\"gender\" x-select-extend=\"\" [(ngModel)]=\"model.gender\" " );
            result.Append( "[data]=\"[{'text':'util.female','value':1,'sortId':1},{'text':'util.male','value':2,'sortId':2}]\"" );
            result.Append( ">" );
            result.Append( "<label *ngFor=\"let item of x_gender.options\" nz-radio=\"\" [nzDisabled]=\"item.disabled\" [nzValue]=\"item.value\">" );
            result.Append( "{{item.text}}" );
            result.Append( "</label>" );
            result.Append( "</nz-radio-group>" );
            result.Append( "</nz-form-control>" );
