...
using Util.Applications.Fakes;
using Util.Applications.Models;
using Util.Applications.Services;
using Xunit;
...
namespace Util.Applications.Tests {
    ...
    public class ProductServiceTest {
        ...
        public async Task TestExtraProperties_6() {
            ...
            //添加实体
            var dto = new ProductDto();
            dto.TestProperties.Add( new ProductItem2 { Code = code, Name = oldName } );
            dto.TestProperties.Add( new ProductItem2 { Code = code, Name = oldName } );
            var id = await _service.CreateAsync( dto );

            //验证
            var oldDto = await _service.GetByIdAsync( id );
            Assert.Equal( 2, oldDto.TestProperties.Count );

            //修改实体
            oldDto.TestProperties[0].Name = newName;
            oldDto.TestProperties[1].Name = newName;
            oldDto.TestProperties.Add( new ProductItem2 { Code = code, Name = name3 } );
            await _service.UpdateAsync( oldDto );
